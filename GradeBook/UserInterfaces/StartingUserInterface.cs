...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            bool x = Convert.ToBoolean(parts[2]);

            if (name == "standard")
            {
                StandardGradeBook GradeBook = new StandardGradeBook(name,x);
                GradeBookUserInterface.CommandLoop(GradeBook);
            }
            else if (name == "ranked")
            {
                RankedGradeBook GradeBook = new RankedGradeBook(name,x);
                GradeBookUserInterface.CommandLoop(GradeBook);

            }
            else if (parts[2] != "true" || parts[2] != "false")
            {
